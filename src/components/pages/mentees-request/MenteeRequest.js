...
import { 
  ...
  FiMail, FiTarget, FiAward, FiUser, FiMoreVertical 
} from "react-icons/fi";
...
export default function MenteeRequest() {
  ...
  const updateStorage = (updatedList) => {
    ...
    // Also sync dashboard counter trigger
  ...
  const handleAccept = (id) => {
    // 1. Find the target mentee profile details
    const target = requests.find(r => r.id === id);
    if (target) {
      // 2. Add them to active mentees workspace storage
    ...
    // 3. Filter out from outstanding list
  ...
  const filteredRequests = requests.filter(req => {
    ...
    return fullName.includes(searchQuery.toLowerCase()) || 
           req.emailAddress.toLowerCase().includes(searchQuery.toLowerCase());
  });
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 md:p-8 max-w-5xl mx-auto text-slate-700 animate-fadeIn">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 border-b border-slate-100 pb-6">
          ...
          <div className="text-center sm:text-left space-y-1">
            <h2 className="text-xl font-bold text-slate-900">{selectedMentee.firstName} {selectedMentee.lastName}</h2>
            <p className="text-sm text-slate-500 font-medium">{selectedMentee.designation}</p>
        ...
        {/* Detailed Bio & Area of Improvement blocks */}
        <div className="py-6 space-y-6">
          <div className="space-y-2">
            <h3 className="text-sm font-bold text-slate-800">Bio/Summary</h3>
            <p className="text-xs text-slate-500 leading-relaxed max-w-4xl">
              {selectedMentee.bio || "A driven and resourceful professional with a passion for learning, personal development, and workflow optimizations. Seeking active guidance under senior leadership to hone technical skill frameworks, build structural career milestones, and maximize collective contributions towards organizational ecosystems."}
            </p>
          </div>
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto p-1 text-slate-800">
      ...
      {/* Structural Section Header Banner */}
      <div>
        <h1 className="text-xl font-bold text-slate-900">Mentee Request</h1>
        <p className="text-xs text-slate-400 mt-0.5">Unlocking Potential: Responding to Mentee Requests</p>
      </div>

      {/* VIEW 1: EMPTY INBOX CANVAS LAYER */}
      {requests.length === 0 ? (
        <div className="bg-white border border-slate-100 rounded-2xl min-h-[500px] flex flex-col items-center justify-center text-center p-6 animate-fadeIn">
          <div className="max-w-xs mx-auto space-y-4">
            {/* Custom Illustrated Folder Graphic Placeholder */}
            <div className="relative w-32 h-32 mx-auto flex items-center justify-center text-indigo-100 bg-slate-50 rounded-full border border-slate-100/60">
              ...
              <span className="absolute bottom-6 right-6 flex h-3 w-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs text-slate-500">
            ...
            <div className="relative max-w-sm w-full sm:w-64">
              <input 
                ...
                className="w-full bg-slate-50/80 border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-slate-700 placeholder-slate-400 outline-none focus:bg-white focus:border-indigo-400 transition"
              />
              <FiSearch className="absolute left-3 top-3 text-slate-400" size={14} />
            </div>
          </div>

          {/* Core Structured Data Sheets Matrix */}
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-600 border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredRequests.slice(0, entriesCount).map((req) => (
                  <tr key={req.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="py-3 px-3 font-medium text-slate-700">{req.lastName}</td>
                    <td className="py-3 px-3 text-slate-400 font-medium">{req.designation}</td>
                    <td className="py-3 px-3 text-slate-500 font-mono">{req.emailAddress}</td>
                    ...
                    <td className="py-3 px-3 text-center relative">
                      {/* Action Pill Custom Trigger Button */}
                      <button 
                      ...
                      >
                        Action <FiChevronDown size={12} className={`transition-transform duration-200 ${openDropdownId === req.id ? "rotate-180" : ""}`} />
                      </button>

                      {/* VIEW 3: INLINE SELECTIVE CONTEXT POPUP DROPDOWN POPPING ELEMENT */}
                      {openDropdownId === req.id && (
                        <div 
                          ref={dropdownRef}
                          className="absolute right-3 top-11 bg-white border border-slate-100 shadow-xl rounded-xl py-1.5 w-36 text-left z-30 font-medium text-slate-600 border-t-2 border-t-slate-300 animate-dropdown"
                        >
                          <button 
